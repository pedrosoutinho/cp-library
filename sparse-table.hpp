...
template <class Monoid> struct Sparse_Table {
    using MX = Monoid;
    ...
    template <typename F> Sparse_Table(int n, F f) { build(n, f); }
    Sparse_Table(const V<X>& v) { build(v); }
    ...
    template <class F> int max_right(const F check, int L) {
