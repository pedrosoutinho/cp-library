...
struct Remove_One_Edge_Connectivity {
    ...
    template <typename GT> Remove_One_Edge_Connectivity(GT& G) { build(G); }

    template <typename GT> void build(GT& G) {
        N = G.N, M = G.M;
        edge.resize(M);
        ...
        auto dfs = [&](auto& dfs, int v, int last_e) -> void {
            ...
            for (auto&& e : G[v]) {
                ...
                if (root[e.to] == -1) {
                    root[e.to] = root[v];
                    dfs(dfs, e.to, e.id);
                    ckmin(low[v], low[e.to]);
                    bridge[e.id] = (low[e.to] == LID[e.to]);
                } else {
                    ckmin(low[v], LID[e.to]);
                }
            }
            RID[v] = p;
        };
        rep(v, N) {
            if (root[v] == -1) {
                n_comp_base++, root[v] = v, dfs(dfs, v, -1);
            }
        }
        for (auto& [a, b] : edge) {
