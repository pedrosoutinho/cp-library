...
// NODES = 40 * N
// DynamicSegTree<Monoid_Add<int>, false, 40 * N> seg(-LIM, LIM);
template <typename Monoid, bool PERSISTENT, int NODES> struct DynamicSegTree {
    using MX = Monoid;
    ...
    DynamicSegTree(
        long long L0,
        long long R0,
        F default_prod = [](long long l, long long r) -> X { return MX::unit(); })
    ...
    template <typename F> long long max_right(np root, F check, long long L) {
        assert(pid && root && L0 <= L && L <= R0 && check(MX::unit()));
        X x = MX::unit();
        return max_right_rec(root, check, L0, R0, L, x);
    }

    template <typename F> long long min_left(np root, F check, long long R) {
