...
template <typename GT> vector<int> incremental_scc(GT& G) {
    static_assert(GT::is_directed);
    int N = G.N, M = G.M;
    vector<int> merge_time(M, infty<int>);
    ...
    vector<int> new_idx(N, -1);
    // L 時点ではサイクルには含まれず, R 時点では含まれる
    auto dfs = [&](auto& dfs, vector<tuple<int, int, int>>& dat, int L,
                   int R) -> void {
        ...
        vector<tuple<int, int, int>> dat1, dat2;
        for (auto [i, a, b] : dat) {
            a = new_idx[a], b = new_idx[b];
            if (i < M) {
                if (comp[a] == comp[b]) {
                    ckmin(merge_time[i], M), dat1.eb(i, a, b);
                } else {
