...
// 単に S の元の列を管理する
// If persistent, use NODES >= 10 * N, and use reset() if pid >= NODES
// If not persistent, use NODES >= N
template <typename S, bool PERSISTENT, int NODES> struct RBST {
    struct Node {
        Node *l, *r;
        S s;
        u32 size;
    ...
    template <typename F> pair<np, np> split_max_right(np root, const F check) {
