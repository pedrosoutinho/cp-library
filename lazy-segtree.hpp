...
template <typename ActedMonoid> struct Lazy_SegTree {
    ...
    template <typename F> Lazy_SegTree(int n, F f) { build(n, f); }
    Lazy_SegTree(const V<X>& v) { build(v); }
    ...
    template <typename F> int max_right(const F check, int l) {
        assert(0 <= l && l <= n);
    ...
    template <typename F> int min_left(const F check, int r) {
        assert(0 <= r && r <= n);
        assert(check(MX::unit()));
        if (r == 0) return 0;
        r += size;
