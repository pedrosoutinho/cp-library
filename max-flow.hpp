...
template <typename Cap> struct MaxFlow {
        ...
        */
        ...
        auto find_path = [&](int s, int t, Cap lim) -> Cap {
            ...
            auto dfs = [&](auto& dfs, int v, Cap f) -> Cap {
                if (v == t) return f;
                for (int& i = prog[v]; i < sz(edges[v]); ++i) {
                    auto& e = edges[v][i];
    ...
    // frm, to, flow
    V<tuple<int, int, Cap>> get_flow_edges() {
        V<tuple<int, int, Cap>> res;
        rep(frm, N) {
    ...
    pair<Cap, V<int>> cut() {
        flow();
        V<int> res(N);
        rep(v, N) res[v] = (level[v] >= 0 ? 0 : 1);
        return {flow_ans, res};
    }

    // O(F(N+M)) くらい使って経路復元
    // simple path になる
    V<V<int>> path_decomposition() {
        flow();
        auto edges = get_flow_edges();
        V<V<int>> TO(N);
        for (auto&& [frm, to, flow] : edges) {
            rep(flow) TO[frm].eb(to);
        }
        V<V<int>> res;
        V<int> vis(N);
    ...
    void debug() {
        ps("source", source);
        ps("sink", sink);
        ps("edges (frm, to, cap, flow)");
        rep(v, N) {
            for (auto& e : edges[v]) {
                if (e.cap == 0 && e.flow == 0) continue;
                ps(v, e.to, e.cap, e.flow);
            }
        }
    }
